...
// neighbor reported its management address as
// "10.237.0.39,fe80::3a10:f0ff:fecf:a500" -- an IPv4 and an IPv6
// link-local address joined by a bare comma on one line. Untouched, the
// whole string became the literal dial target and failed with a "no such
// host" lookup error indistinguishable from a genuine DNS miss.
func TestPrimaryAddressPicksIPv4FromACommaJoinedPair(t *testing.T) {
	got := primaryAddress("10.237.0.39,fe80::3a10:f0ff:fecf:a500")
	if got != "10.237.0.39" {
		t.Errorf("primaryAddress = %q, want %q", got, "10.237.0.39")
	}
}
