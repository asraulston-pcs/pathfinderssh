...
func (s *Stream) Feed(data string) {
	for i := 0; i < len(data); {
		switch s.state {
		case StateGround:
			// In UTF-8 mode, any byte >= 0x80 is part of a multibyte sequence,
			// never a single-byte C1 control. This matters because 0x9B is both
			// the C1 CSI introducer AND a valid UTF-8 continuation byte (the
			// continuation range is 0x80-0xBF), and 0x9D is likewise OSC_C1.
			// Glyphs whose bytes include those values (e.g. braille U+281B, whose
			// encoding ends in 0x9B) would otherwise be split and decoded as
			// U+FFFD. Consume the whole high-byte run as text instead.
			if s.useUTF8 && data[i] >= 0x80 {
				start := i
			...
			default:
				if handler, ok := s.basic[char]; ok {
					...
					for i < len(data) {
						ch := data[i]
						// Stop at any control character or escape. 0x9B (C1 CSI) is
						// only a control in non-UTF-8 mode; in UTF-8 it is a
						// continuation byte, so excluding it here keeps multibyte
						// glyphs that contain it from being split.
						if ch < 0x20 || ch == 0x7f || ch == 0x1b || (!s.useUTF8 && ch == 0x9b) {
							break
						}
						i++
					}
		...
		case StateEscape:
			...
			case "(", ")":
				...
				s.state = StateGround
			case "P", "X", "^", "_":
				// DCS / SOS / PM / APC. All four introduce a STRING that runs
				// until ST, and none of them are display content.
				//
				// Before this existed they fell through to the default branch,
				// which returns to ground - so the payload was DRAWN. vim's
				// capability query (ESC P + q <hex> ESC \\) therefore printed
				// "+q544e" onto the screen at the cursor, and every glyph it
				// wrote also ADVANCED the cursor, displacing whatever vim drew
				// next on that row. Nothing reported an error, and the only
				// symptom was stray characters and a shifted line.
				s.state = StateString
			default:
		...
		case StateString:
			// Consume to ST. Accepted terminators are ESC \\ (ST_C0) and BEL,
			// which xterm tolerates.
			//
			// The 8-bit C1 form (0x9C) is DELIBERATELY not accepted: in UTF-8
			// mode 0x9C is a legal continuation byte, so honouring it would cut
			// a string short in the middle of a multi-byte glyph. Same reasoning
			// as the high-byte run in StateGround. Applications that have been
			// told the terminal is UTF-8 send the 7-bit form.
			//
			// The state lives on the Stream, so a string split across transport
			...
			case char == ESC:
				if i+1 < len(data) && data[i+1] == '\\' {
					i++ // consume the backslash too
					s.state = StateGround
				} else {
					// An ESC that is not ST aborts the string, and whatever
					// follows is a fresh sequence. Do not swallow it.
					s.state = StateEscape
				}
			}
			i++
...
func (s *Stream) dispatchCSI(handler string, params []int, private bool) {
	...
	case "cursor_up", "cursor_down", "cursor_forward", "cursor_back",
		...
		if count > 9999 {
			count = 9999
			dlogf("STREAMS DEBUG: Clamped %s count from %d to 9999", handler, params[0])
		}

		// DEBUG: Log cursor movements
		dlogf("STREAMS DEBUG: Executing %s(count=%d)", handler, count)
	...
	case "cursor_position":
		...
		dlogf("STREAMS DEBUG: CursorPosition(line=%d, column=%d)", line, column)
	...
	case "cursor_to_column":
		...
		dlogf("STREAMS DEBUG: CursorToColumn(column=%d)", column)
	...
	case "cursor_to_line":
		...
		dlogf("STREAMS DEBUG: CursorToLine(line=%d)", line)
