...
func (s *NativeScreen) CursorPosition(line, column int) {
	...
	dlogf("CursorPosition: DECOM=%v, scrollRegion=%v, final pos=(%d,%d)",
...
func (s *NativeScreen) SelectGraphicRendition(params []int) {
	...
	for i := 0; i < len(params); i++ {
		...
		// Bright (AIXTERM) foreground colors (90-97). Named to match the
		// display palette's bright_* keys (and brightenName's bold promotion),
		// so they resolve through the same theme-aware mapping.
		case 90:
			s.cursor.Attrs.Fg = "bright_black"
		case 91:
			s.cursor.Attrs.Fg = "bright_red"
		case 92:
			s.cursor.Attrs.Fg = "bright_green"
		case 93:
			s.cursor.Attrs.Fg = "bright_yellow"
		case 94:
			s.cursor.Attrs.Fg = "bright_blue"
		case 95:
			s.cursor.Attrs.Fg = "bright_magenta"
		case 96:
			s.cursor.Attrs.Fg = "bright_cyan"
		case 97:
			s.cursor.Attrs.Fg = "bright_white"
		// Bright (AIXTERM) background colors (100-107).
		case 100:
			s.cursor.Attrs.Bg = "bright_black"
		case 101:
			s.cursor.Attrs.Bg = "bright_red"
		case 102:
			s.cursor.Attrs.Bg = "bright_green"
		case 103:
			s.cursor.Attrs.Bg = "bright_yellow"
		case 104:
			s.cursor.Attrs.Bg = "bright_blue"
		case 105:
			s.cursor.Attrs.Bg = "bright_magenta"
		case 106:
			s.cursor.Attrs.Bg = "bright_cyan"
		case 107:
			s.cursor.Attrs.Bg = "bright_white"
		// Extended color: 256-color (38;5;n / 48;5;n) and 24-bit truecolor
		// (38;2;r;g;b / 48;2;r;g;b). 256-color is encoded as "color<N>" and
		// truecolor as "#rrggbb"; the display layer decodes both.
		case 38, 48:
			isFg := params[i] == 38
			if i+1 < len(params) {
				switch params[i+1] {
				case 5: // 256-color: <38|48>;5;n
					if i+2 < len(params) {
						name := color256ToString(params[i+2])
						if isFg {
				...
				case 2: // truecolor: <38|48>;2;r;g;b
					if i+4 < len(params) {
						name := rgbToString(params[i+2], params[i+3], params[i+4])
						if isFg {
							s.cursor.Attrs.Fg = name
						} else {
							s.cursor.Attrs.Bg = name
...
// color256ToString encodes a 256-color palette index as "color<N>". The display
// layer decodes it (indices 0-15 -> theme palette, 16-231 -> 6x6x6 cube,
// 232-255 -> grayscale ramp) so the engine stays display-agnostic.
func color256ToString(n int) string {
	return fmt.Sprintf("color%d", n)
}

// rgbToString encodes a 24-bit truecolor triple as "#rrggbb" (channels clamped
// to 0-255). The display layer decodes the leading '#'.
	...
	for _, mode := range modes {
		if private {
			...
			case 6: // DECOM - Origin mode
				...
				dlogf("DECOM origin mode SET - cursor moved to (%d, %d)", s.cursor.X, s.cursor.Y)
			}
		} else {
			// Standard modes
			switch mode {
	...
	for _, mode := range modes {
		if private {
			...
			case 6: // DECOM - Origin mode
				...
				s.cursor.X = 0
				s.cursor.Y = 0
				dlogf("DECOM origin mode RESET - cursor moved to (0, 0)")
			}
		} else {
			// Standard modes
			switch mode {
...
func (s *NativeScreen) SetMargins(top, bottom int) {
	dlogf("SetMargins called: top=%d, bottom=%d (screen: %dx%d)", top, bottom, s.columns, s.lines)
	...
	if top == 0 && bottom == 0 {
		...
		s.scrollTop = 0
		s.scrollBottom = s.lines - 1
		dlogf("Reset scroll region to full screen: 0-%d", s.lines-1)
	} else {
		...
		dlogf("Set scroll region: %d-%d (0-based)", s.scrollTop, s.scrollBottom)
	}

	// Move cursor to top-left of scroll region (vim expects this)
	s.cursor.X = 0
	s.cursor.Y = s.scrollTop
	dlogf("Moved cursor to scroll region origin: (%d, %d)", s.cursor.X, s.cursor.Y)
}
...
func (s *NativeScreen) Resize(newCols, newLines int) {
	...
	dlogf("Screen resized to %dx%d, scroll region: %d-%d", newCols, newLines, s.scrollTop, s.scrollBottom)
}
