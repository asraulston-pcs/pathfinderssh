...
type AlternateScreen struct {
	*HistoryScreen
	...
	// Alternative screen state. Cell data lives in the two stores; only
	// cursor and tab stops need saving alongside.
	mainStore    *RowStore
	mainCursor   Cursor
...
func NewAlternateScreen(columns, lines, maxHistory int) *AlternateScreen {
	...
	a.altStore = NewRowStore(columns, lines, 0)
...
func (a *AlternateScreen) switchToAlternate() {
	...
	// Swap in a store with no scrollback budget rather than clearing and
	// re-pointing parallel buffers.
	if a.altStore == nil {
		a.altStore = NewRowStore(a.columns, a.lines, 0)
	} else {
		a.altStore.Resize(a.columns, a.lines)
		a.altStore.Reset()
	}
	a.mainStore = a.HistoryScreen.SwapStore(a.altStore)
...
func (a *AlternateScreen) switchToMain() {
	...
	a.altStore = a.HistoryScreen.SwapStore(a.mainStore)
	a.cursor = a.mainCursor
