...
func TestBasicStream(t *testing.T) {
	...
	if err != nil {
		// The differential tests compare gopyte against Python pyte via a
		// sidecar process. Without an interpreter and pyte_server.py there
		// is nothing to compare against -- that is a missing harness, not a
		// failing engine.
		t.Skipf("pyte differential harness unavailable: %v", err)
	}
	defer screen.Close()
