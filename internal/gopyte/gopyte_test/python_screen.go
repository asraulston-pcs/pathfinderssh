...
func NewPythonScreen(columns, lines int) (*PythonScreen, error) {
	// Try to find a Python executable. Most distributions ship only
	// "python3"; the bare "python" name is often absent entirely.
	pythonCmd := ""
	...
	if pythonCmd == "" {
		return nil, fmt.Errorf("no python3 or python interpreter found in PATH")
	}
